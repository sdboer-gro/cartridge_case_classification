...
# LICENSE file in the License directory of this source tree.
#
...
def get_args(parser):
    ...
    parser.add_argument("--categories", type=int, default=6, choices=[6, 11, 16, 23]) # 6 and 23 -> cartridge_case, 11 and 16 -> cartridge_case_full
    ...
    parser.add_argument("--model", type=str, default="bow", choices=["img", "mmbt"])
    ...
    parser.add_argument("--task", type=str, default="cartridge_case", choices=["cartridge_case", "cartridge_case_full"])
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    with torch.no_grad():
        losses, preds, tgts, confidence_scores, logits, input_images = [], [], [], [], [], []
        for batch in data:
            loss, out, tgt = model_forward(i_epoch, model, args, criterion, batch)
            losses.append(loss.item())

            pred = torch.nn.functional.softmax(out, dim=1).argmax(dim=1).cpu().detach().numpy()
            logits.append(torch.nn.functional.softmax(out, dim=1).cpu().detach().numpy())
            for s in torch.nn.functional.softmax(out, dim=1).max(dim=1).values.cpu().detach().numpy():
            ...
            _, _, _, img, _ = batch
            input_images.append(img.cpu().detach().numpy().flatten())
    ...
    if store_preds:
        ...
        with open(os.path.join(args.savedir, "test_conf_score.txt"), "w") as fw:
            fw.write("\n".join([str(x) for x in confidence_scores]))
        np.save(os.path.join(args.savedir, 'test_logits'), np.array(np.vstack(logits)))
        np.save(os.path.join(args.savedir, 'input_images_flattened'), np.array(np.vstack(input_images)))
